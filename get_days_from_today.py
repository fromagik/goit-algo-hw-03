...
def get_days_from_today(date): #Створюємо функцію
    while True:
        try: #Обробка винятків 
            ...
            return f"Різниця між вказаною датою {user_date.date()} та сьогодні {currant_date.date()} складає {difference_date.days} днів" # Завершення циклу
...
date = input('Введіть дату в форматі РРРР-ММ-ДД: ') 
print(get_days_from_today(date)) # Виклик функції 